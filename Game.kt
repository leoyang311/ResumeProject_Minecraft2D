 ...
 */
object Game {
    private val blockGrid = Array(NUM_COLS) {
        Array<Block>(NUM_ROWS) {
            Sand()
        }
    }

    private val entityGrid = Array(NUM_COLS) {
     ...
     */
    fun place(entity: Entity, x: Int, y: Int) {
        require(isEmpty(x, y)) {
    ...
    fun getEntity(x: Int, y: Int): Entity? {
        return if (x in 0..NUM_COLS && y in 0..NUM_ROWS)
            entityGrid[x][y]
        else
            null
    }
     ...
     */
    fun getImage(x: Int, y: Int): ColorBuffer? =
        if (x in 0..NUM_COLS && y in 0..NUM_ROWS)
     ...
     */
    fun calculateDistance(entity1: Entity, entity2: Entity): Double {
